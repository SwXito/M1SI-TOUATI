...
constexpr int   NUM_BALASTS_STRAIGHT     = 5;
constexpr int   NUM_BALASTS_CURVED     = 3;
...
constexpr float BALAST_SPACING           = (BALAST_END_X - BALAST_START_X) / (NUM_BALASTS_STRAIGHT  - 1);
...
constexpr float POS_X_RAIL_CURVE     =  20.0f;
...
enum TrackType { STRAIGHT, CURVED_IN, CURVED_OUT };
...
private:
    void createStraightRail(bool);
    void createCurvedRail(int, float, bool);
    std::vector<TrackType> classifyTrack(const std::vector<std::pair<int,int>>&);
    int nb_object;
